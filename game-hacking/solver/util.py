# Support code. These are things you would have to reverse engineer from `game.js`.

from base64 import b64encode, b64decode
import asyncio
import json
import random
import urllib
import zlib

SBOX = [
    
]


def serialize(data):
    encoded = bytearray(json.dumps(data).encode())
    nonce = random.choice(SBOX)
    key = nonce
    for i, c in enumerate(encoded):
        encoded[i] = c ^ key
        key = SBOX[key]
    packet = bytes([nonce]) + encoded
    return b64encode(packet).decode()


def deserialize(chunk):
    chunk = b64decode(chunk)
    nonce = chunk[0]
    key = nonce
    chunk = bytearray(chunk[1:])
    for i, c in enumerate(chunk):
        chunk[i] = c ^ key
        key = SBOX[key]
    return json.loads(chunk)


def split_base64_chunks(bin):
    chunks = []
    while len(bin) > 0:
        for i in range(len(bin)):
            if bin[i] not in b"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=":
                chunks.append(bin[:i])
                bin = bin[i + 1:]
                break
        else:
            break
    chunks.append(bin)
    return chunks


def parse_world(packet):
    world = { "tilemaps": dict(), "mobs": dict() }
    for encoded in split_base64_chunks(urllib.parse.unquote_to_bytes(packet.get("blob"))):
        obj = json.loads(b64decode(encoded))
        world["tilemaps"][obj["location"]] = json.loads(
            zlib.decompress(
                b64decode(obj["tilemap"])
            ).decode()
        )
        world["mobs"][obj["location"]] = obj["mobs"]
    return world


def encode_world(location, tilemap, mobs):
    obj = {
        "location": location,
        "tilemap": b64encode(
            zlib.compress(json.dumps(tilemap).encode())
        ).decode(),
        "mobs": mobs,
    }
    return b64encode(json.dumps(obj).encode())
